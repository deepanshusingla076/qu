...
echo [4/8] Checking User Service (8083)...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:8083/actuator/health' -TimeoutSec 5; if($response.StatusCode -eq 200) { Write-Host 'User Service: RUNNING' -ForegroundColor Green } } catch { Write-Host 'User Service: NOT RESPONDING' -ForegroundColor Red }"

echo [5/8] Checking Question Bank Service (8081)...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:8081/actuator/health' -TimeoutSec 5; if($response.StatusCode -eq 200) { Write-Host 'Question Bank Service: RUNNING' -ForegroundColor Green } } else { Write-Host 'Question Bank Service: ERROR' -ForegroundColor Red } } catch { Write-Host 'Question Bank Service: NOT RESPONDING' -ForegroundColor Red }"

echo [6/8] Checking Result Service (8082)...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:8082/actuator/health' -TimeoutSec 5; if($response.StatusCode -eq 200) { Write-Host 'Result Service: RUNNING' -ForegroundColor Green } } catch { Write-Host 'Result Service: NOT RESPONDING' -ForegroundColor Red }"
